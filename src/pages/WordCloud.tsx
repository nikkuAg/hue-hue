...
export default function WordCloud() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("blessings-cloud-changes").on("postgres_changes", {
      event: "INSERT",
      schema: "public",
      table: "blessings"
    }, payload => {
  ...
  const fetchRecentBlessings = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("blessings").select("id, message").order("created_at", {
        ascending: false
      }).limit(6);
  ...
  const fetchBlessings = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("blessings").select("*").order("created_at", {
        ascending: false
      });
      if (error) throw error;
      setBlessings(data || []);
  ...
  const wordCloudData = useMemo(() => {
    ...
    blessings.forEach(blessing => {
      const words = blessing.message.toLowerCase().replace(/[^\w\s]/g, "").split(/\s+/).filter(word => word.length > 3); // Only words longer than 3 characters
      ...
      <div className="max-w-6xl mx-auto relative z-10 space-y-8">
        ...
        <header className="text-center mb-8">
          <div className="flex justify-center gap-4 mb-4">
            ...
            <Button onClick={() => navigate("/blessings")} variant="outline" className="border-coral text-coral hover:bg-coral hover:text-white">
        ...
        {cloudToasts.map((toast, index) => <CloudToast key={toast.id} message={toast.message} index={index} delay={index * 200} // Stagger the appearance
      onRemove={() => {
        setCloudToasts(prev => prev.filter(t => t.id !== toast.id));
      }} />)}

        {/* Word Cloud */}
        {loading ? <div className="text-center text-muted-foreground">Loading blessings...</div> : blessings.length > 0 ? <Card className="p-6 md:p-8 shadow-card border-teal/20 bg-white">
            ...
            fov: 60
                ...
                <Suspense fallback={null}>
                  ...
                  <OrbitControls enableZoom={true} enablePan={false} minDistance={8} maxDistance={20} autoRotate autoRotateSpeed={0.5} />
                </Suspense>
              </Canvas>
            </div>
          </Card> : <Card className="p-8 text-center shadow-card border-teal/20 bg-gradient-to-br from-cream to-white-smoke">
            ...
            <Button onClick={() => navigate("/blessings")} className="mt-4 bg-gradient-to-r from-coral to-peach text-navy hover:opacity-90">
              <Send className="mr-2 h-4 w-4" />
              Send a Blessing
            </Button>
          </Card>}
